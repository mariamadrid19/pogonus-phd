...
samtools index $IN/$(echo "${SAMPLE[indID]}").nieuPrim.filtered.sorted.nd.bam $IN/$(echo "${SAMPLE[indID]}").nieuPrim.filtered.sorted.nd.bam.bai
...
bcftools mpileup -q 20 -Q 20 -C 50 -r $SCAF -f $REF $IN/$(echo "${SAMPLE[indID]}").nieuPrim.filtered.sorted.nd.bam | bcftools call -c - | bamCaller.py 30 $OUT/$(echo "${SAMPLE[indID]}")_$SCAF.mask.bed.gz | gzip -c > $OUT/$(echo "${SAMPLE[indID]}")_$SCAF.vcf.gz
