...
for CHROM in "${!inverted_regions[@]}"
do
    # Get the specific region for the current chromosome
    REGION="${inverted_regions[$CHROM]}"

    # Run bcftools and subsequent commands for the specified region
    ...
    gzip -c > $INV_OUT/$(echo "${SAMPLE[indID]}")_${CHROM}_${REGION}.vcf.gz
    ...
    $INV_OUT/$(echo "${SAMPLE[indID]}")_${CHROM}_${REGION}.vcf.gz > $INV_OUT/$(echo "${SAMPLE[indID]}")_${CHROM}_${REGION}.txt

    # Add the generated .txt file to the COMMAND variable
    COMMAND="$COMMAND $INV_OUT/$(echo "${SAMPLE[indID]}")_${CHROM}_${REGION}.txt"
done

# Run MSMC2
msmc2_Linux -t 12 -o $INV_OUT2/$(echo "${SAMPLE[indID]}") $COMMAND
...
do
    python MSMC_plotInput.py -I $INV_OUT2/$FINAL.final.txt -u 2.1e-09 -g 1 > $INV_OUT2/$FINAL.final.Rin.txt
done
